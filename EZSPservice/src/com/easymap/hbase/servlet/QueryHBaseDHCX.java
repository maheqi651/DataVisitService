...
public class QueryHBaseDHCX implements Action {
			...
			if (str != null) {
				...
				for(int i = 0; i < eItem.size(); i++) {
					...
					if (eIName.getTextTrim().equals("DataObjectCode")) {
						...
						where =e.element("Value").elements("Data");
...
/*						if (order.endsWith(","))
			...
			if(request.getAttribute(Tools.FLAG)!=null)   //提供运维验证标记;
			{
	            ...
	            SRVQueryHBaseDHCXModle sqdr = new SRVQueryHBaseDHCXModle();
				try {
					s += sqdr.getXML(senderID, dataObjectCode, themeCode,methodName,
							where, fields, order, startRow, max,istotal);
				} catch (Exception e) 
				{
					e.printStackTrace();
				}
