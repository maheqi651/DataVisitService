			...
			if(request.getAttribute(Tools.FLAG)!=null)   //提供运维验证标记;
			{
			   ...
			   if(isTimeOut)
			   {
				...
				//= request.startAsync(request,response);
			       ...
			       //executor.awaitTermination(timeOutNum, TimeUnit.MILLISECONDS);
			        
			       /* executor.execute(new com.easymap.base.timeout.AsyncRequestProcessor(response, senderID, 
				 ...
				 if(!resultsub.isFlag())
				  {
					    ...
					    s+="<ERR>近期访问流量超出，系统判定：从"+dataformat.format(resultsub.getBlockTime())+"起,禁止访问"+resultsub.getChkBlockTimeSpan()+"分钟</ERR>";
					   ...
					   ExecutorService executor=Executors.newFixedThreadPool(1);
					    try{ 
					    	 executor.submit(new com.easymap.base.timeout.AsyncRequestProcessor(response, senderID, 
						        		dataObjectCode, themeCode,methodName,
										where, fields, order, start, max,istotal)).get(timeOutNum, TimeUnit.MILLISECONDS);
						      
					    }catch(TimeoutException e)
					    {
					    	//e.printStackTrace();
							s += "<ERR>Request TimeOut</ERR>";
							response.setContentType("text/html;charset=utf-8");
							PrintWriter out = null;
							try {
								out = response.getWriter();
